...
exports.forgotpassword = asyncHandler(async (req, res, next) => {
  ...
  const message = `Σας ενημερώνουμε ότι έγινε αίτημα για επανέκδοση κωδικού πρόσβασης στον λογαριασμό που διατηρείτε στην διεύθυνση ${process.env.FRONT_DOMAIN}. \n\nΓια την επανέκδοση του κωδικού πρόσβασης, επισκεφτείτε τον σύνδεσμο που ακολουθεί ${resetUrl} \n\nΕαν το αίτημα δεν πραγματοποιήθηκε από εσάς, μπορείτε να αγνοήσετε αυτό το μήνυμα με ασφάλεια. \nΟ παραπάνω σύνδεσμος θα είναι έγκυρος για δέκα λεπτά.`;

  try {
    await sendEmail({
      email: user.email,
      subject: 'Επανέκδοση κωδικού',
      message,
    });
...
exports.resetpassword = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.password.length < 5) {
    return next(
      new ErrorResponse(
        `Ο κωδικός πρόσβασης πρέπει να περιέχει τουλάχιστον 5 ψηφία.`,
        401
      )
    );
  }
