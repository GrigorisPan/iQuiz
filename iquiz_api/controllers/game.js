...
exports.getQuestions = asyncHandler(async (req, res, next) => {
  ...
  try {
    questions_arch = fs.readFileSync(`${__dirname}/quiz${otp}.xml`, 'utf-8');

    if (questions_arch) {
      const data = questions_arch;
      // convert XML to JSON
      xml2js.parseString(
        data,
    ...
    return next(new ErrorResponse('Σφάλμα OTP', 404));
  }
});

//  @desc     Check OTP
//  @route    GET /api/v1/game/check/:otp
//  @access   Private  (Teacher & Admin)
exports.checkQuestions = asyncHandler(async (req, res, next) => {
  ...
  if (type === 1 || type === 2) {
    ...
    return next(
      new ErrorResponse(
        `Ο χρήστης δεν έχει δικαίωμα να λάβει τις ερωτήσεις`,
        401
      )
    );
  }
